    ...
    ($handler_getter: expr, $name: ident for $ref: ident<$ptr: ty>, [
        ...
        paste::paste! {
            ...
            extern "C" fn [< $signal_name:snake _handler>](obj_ptr: *mut std::ffi::c_void, __internal_calldata: *mut libobs::calldata_t) {
                ...
                // Wraps this with tokio::task::spawn_blocking if the feature blocking is disabled
                let obj_ptr = $crate::unsafe_send::SendableComp(obj_ptr as $ptr);
                $crate::wrap_with_spawn_blocking!({
                    let res = res.unwrap();
                    #[cfg(feature="blocking")]
                    let senders = [<$signal_name:snake:upper _SENDERS>].read();
                    #[cfg(not(feature="blocking"))]
                    let senders = [<$signal_name:snake:upper _SENDERS>].blocking_read();
                    let senders = senders.get(&obj_ptr);
                    if senders.is_none() {
